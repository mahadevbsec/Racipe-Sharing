...
const Recipes = () => {
  ...
  const fetchRecipes = async () => {
    try {
      const res = await fetch("https://racipe-sharing.onrender.com/auth/recipe", {
        method: "GET",
        headers: {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(
        `https://racipe-sharing.onrender.com/auth/recipe/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleAddFavorite = async (id) => {
    try {
      const res = await fetch(
        `https://racipe-sharing.onrender.com/auth/likedRecipes/${id}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleSearchChange = async (e) => {
    ...
    if (key.trim().length === 0) {
      ...
      try {
        const res = await fetch(
          `https://racipe-sharing.onrender.com/auth/searchRecipes/${key}`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
